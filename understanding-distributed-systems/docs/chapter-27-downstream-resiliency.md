...
As a rule of thumb, always set timeouts when making network calls, and be wary of third-party libraries that make network callstch/issues/951

> 1“Web APIs: XMLHttpRequest.timeout,” https://developer.mozilla.org/enUS/docs/Web/API/XMLHttpRequest/timeout 

> 2“Add a timeout option, to prevent hanging,” https://github.com/whatwg/fe

tch/issues/951olang/go/issues/24138
...
> 5“net/http: make default configs have better timeouts,” https://github.com/g

olang/go/issues/24138api/system.net.http.httpclient.timeout?view=net-6.0#remarks but don’t expose settings for timeouts.

> 6“HttpClient.Timeout Property,” https://docs.microsoft.com/en-us/dotnet/
...
Actively waiting and retrying failed network requests isn’t the only way to implement retries. In batch applications that don’t have strict real-time requirements, a process can park a failedure/exponential-backoff-and-jitter/

> 8“Exponential Backoff And Jitter,” https://aws.amazon.com/blogs/architect
...
The goal of the circuit breaker is to allow a sub-system to fail without slowing down the caller. To protect the system, calls to the failing sub-system are temporarily blocked. Later, when the subsystem recovers and failures stop, the circuit breaker allows calls to go through again. 
